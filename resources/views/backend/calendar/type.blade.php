            ...
            <div class="content-body">
                ...
                <section class="users-list-wrapper">
                    <div class="users-list-filter px-1">
                        <form action="{{ route('admin.calendar.type.create') }}" method="post">
                            @csrf
                            <div class="row border rounded mb-2">
                                <div class="col-12 col-sm-6 col-lg-3">
                                    <fieldset class="form-group pt-2 mb-0">
                                        <input type="text" name="name" class="form-control" placeholder="Nhập tên loại lịch trình">
                                    </fieldset>
                                    @if($errors->any())
                                        <div style="margin: 0.5rem 0">
                                            <span class="text-danger my-1">*Vui lòng nhập tên loại lịch trình</span>
                                        </div>
                                    @endif
                                </div>
                                    ...
                                    <div class="table-responsive">
                                        <table id="" class="table table-striped">
                                            <thead>
                                                <tr>
                                                    <th>STT</th>
                                                    <th>Tên loại lịch trình</th>
                                                    <th>Mã loại lịch trình</th>
                                                    <th>Thao tác</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                @foreach ($records as $key => $record)
                                                    <tr>
                                                        ...
                                                        <td>{{ $record->id }}</td>
                                                        <td>
                                                            <button data-id="{{ $record->id }}" class="type-edit border-0 bg-transparent" data-toggle="modal" data-target="#update">
                                                                <i class="far fa-edit"></i>
                                                            </button>
                                                            <form style="display: inline-block" method="POST" action="{{ route('admin.calendar.type.delete') }}">
                                                                @csrf
                                    ...
                                    <div class="modal fade" id="update" aria-labelledby="update" aria-hidden="true" tabindex="-1" role="dialog">
                                        <div class="modal-dialog modal-dialog-centered" role="document">
                                            <div class="modal-content">
                                                <form action="{{ route('admin.calendar.type.update') }}" method="post">
                                                    @csrf
                                                    ...
                                                    <div class="modal-body">
                                                        <input id="type-id" type="hidden" name="id" value="">
                                                        <input id="type-name" type="text" name="name" class="form-control">
                                                    </div>
